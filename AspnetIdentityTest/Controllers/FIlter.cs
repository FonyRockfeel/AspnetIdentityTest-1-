    ...
    public class Filters
    {
        ...
        internal ObjectQuery<T> FilterObjectSet<T>(ObjectQuery<T> inputQuery) where T : class
        {
            ...
            foreach (Rule rule in rules)
            {
                ...
                ObjectParameter op;
                if (String.Compare(propertyInfo.PropertyType.FullName,
                        "System.Int32", StringComparison.Ordinal) == 0)
                {
                    op = new ObjectParameter("p" + iParam, Int32.Parse(rule.data));
                }
                else if (String.Compare(propertyInfo.PropertyType.FullName,
                             "System.DateTime", StringComparison.Ordinal) == 0)
                {
                    op = new ObjectParameter("p" + iParam, DateTime.Parse(rule.data));
                }
                else
                {
                    op = new ObjectParameter("p" + iParam, rule.data);
                }
